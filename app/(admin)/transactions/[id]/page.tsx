...
const STATUS_FLOW: BookingStatus[] = [
  'PENDING', 'CHECKING', 'WAITING_PAYMENT', 'PAYMENT_REVIEW', 'IN_PROGRESS', 'TESTING', 'COMPLETED',
];
...
function InfoRow({ label, value, mono, highlight }: { label: string; value: string; mono?: boolean; highlight?: boolean }) {
  return (
    <div className="flex items-start justify-between gap-4 py-2.5 border-b border-gray-700/40 last:border-0">
      ...
      <span className={`text-sm text-right flex-1 ${mono ? 'font-mono text-gray-300' : highlight ? 'text-green-400 font-semibold' : 'text-white'}`}>{value}</span>
    </div>
  );
}
...
export default function TransactionDetailPage() {
  ...
  async function fetchBooking() {
    ...
    try {
      ...
      // Parts endpoint mungkin belum ada di production, handle gracefully
      try {
  ...
  async function handleAddPart(e: React.FormEvent) {
    ...
    try {
      ...
      const [updatedBooking, updatedStocks] = await Promise.all([
        getBooking(id),
        getStocks(),
      ]);
      setBooking(updatedBooking);
      setStocks(updatedStocks);
      if (updatedBooking.totalAmount != null) setAmountInput(updatedBooking.totalAmount.toString());
  ...
  const nextStatus = booking ? getNextStatus(booking.status) : null;  const paymentProofUrl = booking?.paymentProofUrl ? `${IMAGE_BASE_URL}${booking.paymentProofUrl}` : null;
  const canSetAmount = booking?.status === 'PENDING' || booking?.status === 'CHECKING';
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-gray-800 rounded-2xl border border-gray-700/50 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-700/50 flex items-center justify-between">
                ...
                {parts.length > 0 && (
                  <span className="px-2 py-0.5 bg-blue-900/40 text-blue-400 border border-blue-800 text-xs rounded-full font-medium">
                    {parts.length} item
                  </span>
                )}
              </div>
              <div className="p-6 space-y-4">
                ...
                <form onSubmit={handleAddPart} className="flex flex-col sm:flex-row gap-3">
                  <select
                    ...
                    required
                    className="flex-1 px-3 py-2.5 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-red-600 focus:border-transparent"
                  >
                    <option value="">Pilih sparepart...</option>
                    {stocks.filter(s => s.quantity > 0).map((s) => (
                      <option key={s.id} value={s.id} className="bg-gray-800">
                        {s.name} — {formatRupiah(s.price)} (stok: {s.quantity})
                      </option>
                    ))}
                  </select>
                  <input
                    type="number" min="1" required value={partQty}
                    onChange={(e) => setPartQty(e.target.value)}
                    placeholder="Qty"
                    className="w-20 px-3 py-2.5 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-red-600 text-center"
                  />
                ...
                {partError && (
                  <div className="p-3 bg-orange-900/40 border border-orange-700 rounded-lg text-orange-300 text-sm flex items-center gap-2">
                    ...
                    {partError.includes('404') ? 'Fitur sparepart belum aktif di server. Hubungi admin server untuk deploy ulang.' : partError}
                  </div>
                )}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-gray-700/50">
                      ...
                      <span className="text-white font-semibold">
                        {formatRupiah(parts.reduce((s, p) => s + p.priceEach * p.quantity, 0))}
                      </span>
                    </div>
                  </div>
                )}
            ...
            {/* Payment Proof */}
            {paymentProofUrl && (              <div className="bg-gray-800 rounded-2xl border border-gray-700/50 overflow-hidden">
                ...
                <div className="p-6">
                  <button onClick={() => setShowImage(true)}
                    ...
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition flex items-center justify-center">
                      <span className="opacity-0 group-hover:opacity-100 transition text-white bg-black/60 px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-800 rounded-2xl border border-gray-700/50 overflow-hidden">
              ...
              <div className="p-6">
                ...
                ) : booking.status === 'WAITING_PAYMENT' && !paymentProofUrl ? (
                  <div className="flex items-center gap-2 p-3 bg-orange-900/30 border border-orange-800 rounded-lg text-orange-400 text-sm">
